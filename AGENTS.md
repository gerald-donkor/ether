...
2a. **Missing skill coverage is a priority blocker, at every task, stage and process.** If any technology or stack surface the work needs has no applicable installed skill, search the web and the open agent-skill ecosystem for one, verify its source and quality, and install the best trustworthy match before continuing. Do this during prompt preparation as well as execution, not only after an API gap appears. Prefer an official provider or framework skill, then a reputable maintained source; never install an unverified package merely to satisfy this rule. Record every installed skill in the prompt's `## SKILLS USED` manifest. If no trustworthy skill exists, state the search result and use live official documentation under §12 rather than guessing.
2b. **Latest documentation is a priority requirement, not a nicety, and it outranks a loaded skill.** Every surface a task touches must be written against that technology's **current** documentation for the version installed here: Next.js from `node_modules/next/dist/docs/`, a provider from its live docs fetched **this session**, a package API from the `.d.ts` in `node_modules/`. A skill is a starting point and can be stale; where a skill and the live docs disagree, **the live docs and the installed types win, and the disagreement is stated in the prompt file or the commit**. Prefer a live-docs skill (`stripe-docs`) over a static one for provider APIs. For every provider call, event name, SDK option, environment variable and schema field a change introduces, record **what was checked and on what date** in `docs/backend.md` — an API written from memory or from a training-data pattern is the §12 rule 2 failure, whether or not it happens to work.
...
the two Clerk publishable names and `NEXT_PUBLIC_STRIPE_PUBLISHABLE_KEY`:
...
2. **Never write an API you have not verified** in `node_modules/`, a loaded
   ...
   from memory, and Next 16 contradicts almost every tutorial. **Current docs
   for the installed version are the standard** (§1 step 2b): a skill that
   disagrees with the live docs or with `node_modules/` is the stale one.
